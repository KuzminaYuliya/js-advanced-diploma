  ...
  onCellEnter(index) {
    ...
    if (!this.blockedBoard) {
      ...
      if (this.selected) {
        ...
        const allowPositions = getAllowedPositions(allowPosition, allowDistance, boardSize);

        allowDistanceAttack = this.selectedCharacter.character.distanceAttack;
        const allowAttack = getAllowedPositions(allowPosition, allowDistanceAttack, boardSize);
